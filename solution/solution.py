...
def compare_models(prompt: str) -> dict:
    ...
    mini_cost_estimate = (len(mini_response.split()) / 0.75) / 1000 * COST_PER_1K_OUTPUT_TOKENS["gpt-4o-mini"]
    return {
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        try:
            ...
            history = history[-6:]  # Keep last 3 turns (user+assistant)
        except Exception as e:
            print(f"Error occurred: {e}")
            if history and history[-1]["role"] == "assistant":
                history.pop()  # Remove incomplete assistant reply from history
            continue
...
def retry_with_backoff(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # If we've exhausted our max_retries, surface the exception
